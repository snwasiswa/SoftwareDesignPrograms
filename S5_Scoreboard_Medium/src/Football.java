...
public class Football extends Game{
    ...
    public void setCurrentPeriod(int quarter){
        if ( 0 < quarter && quarter <= getLengthPeriod()) {
    ...
    public int addScore(String method, Team team){
        ...
        if(method.equalsIgnoreCase(getScoringMethods().get(3)) && team.equals(getHomeTeam())){
            homeTeamScore += 2;
        }
        if(method.equalsIgnoreCase(getScoringMethods().get(4)) && team.equals(getHomeTeam())){
            homeTeamScore += 2;
        }
        ...
        if(method.equalsIgnoreCase(getScoringMethods().get(3)) && team.equals(getAwayTeam())){
            awayTeamScore += 2;
        }
        if(method.equalsIgnoreCase(getScoringMethods().get(4)) && team.equals(getAwayTeam())){
            awayTeamScore += 2;
        }
        return 0;
    }
    ...
    public ArrayList<String> getScoringMethods(){
        scoringMethods.add("touchdown");
        scoringMethods.add("field goal");
        scoringMethods.add("extra-point");
        scoringMethods.add("two-point conversion");
        scoringMethods.add("safety");
